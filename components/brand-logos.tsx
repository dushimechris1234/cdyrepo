...
export default function BrandLogos() {
  const brands = [
    { name: "Meta", logo: "Meta" },
    { name: "Coca-Cola", logo: "Longtai" },
    { name: "Disney", logo: "DevOps" },
    { name: "Uber", logo: "Uber" },
        ...
        <div className="relative">
          ...
          >
            {[...brands, ...brands].map((brand, index) => (
              <div
                key={index}
                className="flex-shrink-0 w-40 h-16 bg-gray-800/50 dark:bg-gray-200/50 rounded-lg flex items-center justify-center text-3xl font-bold text-white/80 dark:text-gray-900/80"
              >
                {brand.logo}
              </div>
            ))}
